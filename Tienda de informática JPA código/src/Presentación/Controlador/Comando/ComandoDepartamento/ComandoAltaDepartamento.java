...
public class ComandoAltaDepartamento implements ComandoInterfaz {
	...
	public Context execute(Context contextEntrada) throws Exception {
		...
		if(tDepartamento != null)
			idDevuelto = saDepartamento.altaDepartamento(tDepartamento);
		
		if(idDevuelto != -1){
			contextSalida.setEvento(EventoGUI.ALTA_DEPARTAMENTO_OK);
		}		
		else{
			contextSalida.setEvento(EventoGUI.ALTA_DEPARTAMENTO_ERROR);
		}
